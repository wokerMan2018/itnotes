...
server='1.2.3.4' #your server ip or domain(===edit===)
local=0.0.0.0
interface=eth0  #your server interface(===edit===)
subnet=10.0.0.0 #virtual subnet(===edit===)
prefix=24
mask=255.255.255.0
...
function gen_certs() {
  ...
  # sign the Server certificates on the CA
  echo yes | easyrsa sign-req server $crt_name
  cp /etc/easy-rsa/pki/issued/$cert /etc/openvpn/server/

  #dh Diffie-Hellman (DH) parameters file
  openssl dhparam -out /etc/openvpn/server/dh.pem 2048

  #ta Hash-based Message Authentication Code (HMAC) key
  ...
  #sign the Client certificates on the CA
...
cat $log |grep CONTROL |grep --o -E '\[.+\]|ifconfig\s([0-9]+\.?){4}'|sed -n '{s/ifconfig //;p}'
