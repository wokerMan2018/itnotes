...
- `set`  显示/设置shell选项和位置参数的值；显示/设置shell变量的名称和值。
- `declare`  显示/定义已经存在的变量。
- `export`  将变量/函数输出为环境变量。
...
可以使用`export`将变量导出为环境变量，在同一shell下的子shell中均有效：
...
- 使用双引号`""`包裹变量时：
  - `"$*"`将所有的参数解释成一个字符串
  - `"$@"`将参数当作一个参数数组。其将以IFS（默认为空格）来划分字段，但如果空格在引号`""`里面则忽略而不进行划分。

示例：传入参数`a b`，`"$*"`当作`a b`整体，`"$@"`当作`a`和`b`
